...
class Agent:
    ...
    def get_action(self, envInfo, new_im_message=""):
        system_prompt = f'你叫{self.name}，你是{self.persona}，你现在在{self.envName}。根据给出的信息 1.思考（格式为"思考【思考内容】"），2.更新记忆（格式为"更新记忆【新的记忆】"），3.决定你接下来一小时的动作。把这三部分按格式输出。世界中有下列场景：{envs.get_all_envNameStr()}，如果你想在当前场景做一些事，直接说你要做什么（思考不等于行动，行动的具体内容需要在此重复给出）。如果你想移动到不同的场景，用格式"我要前往【场景名】"给出你要到达的场景。注意，不要重复向你当前所在的场景移动！'
        full_context = f"你的记忆是：{self.memory}\n\n你在{self.envName}看到了：{envInfo}，需要特别注意，场景中有你想要互动的人时才能进行互动，不要和不存在的人进行互动。如果你找不到你要互动的人，你可以使用IM给他发送消息，现在你的通讯录中有：{self.get_addressBook_str()}，发送消息用格式“向【联系人名】发送【消息内容】”，如果你的通讯录中没有你想联系的人，你可以发送“向【联系人名】发送【申请好友】。”"
        if new_im_message:
...
class IM:
    ...
    def send_message_by_action(self, fromName, action):
        match = re.search(r'向【(.+?)】发送【(.+?)】', action)
        if match:
            toName = match.group(1)
            message = match.group(2)
            if fromName != '' and toName != '' and fromName != None and toName != None:
